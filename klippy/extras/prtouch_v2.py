...
def _load_config(config):
    ...
    if pr_version == 2:
        ...
        raise config.error("Unsupported prtouch pr_version: %d" % (pr_version,))

    # Register the probe using the named-probe helper, so [prtouch_v2 prtouch]
    # does not replace the global "probe" object used by Cartographer.
    chipname, gcode_prefix = probe.lookup_probe_names(config)
    config.get_printer().add_object(chipname, probe.PrinterProbe(config, vrt))
    return vrt
